// The code has been based upon the Image.h file developed by Epic Games Inc.
...
class SRadialProgressBar;
class UMaterialInstanceDynamic;
class UMaterialInterface;
class USlateBrushAsset;
class UTexture2D;
class ULayerImageAsset;
struct FStreamableHandle;



UCLASS()
class TANKMAYHEM_API URadialProgressBar : public UWidget
{
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category=Appearance)
	TSubclassOf<ULayerImageAsset> LayerImageAssetClass;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category=ProgressBar)
	float ProgressPercentage = 0.0f;

	UPROPERTY()
	FGetFloat ProgressPercentageDelegate;
...
protected:
	TSharedPtr<SRadialProgressBar> MyImage;
...
protected:
	...
	PROPERTY_BINDING_IMPLEMENTATION(float, ProgressPercentage);
};
